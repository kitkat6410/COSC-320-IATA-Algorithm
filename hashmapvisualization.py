...
def tryItABunch(myFn, startN=10, endN=100, stepSize=10, numTrials=20, listMax=10):
    ...
    for n in range(startN, endN, stepSize):
        myHashMap = dict(list(IATACodeHashmap.items())[:n])
...
nValues, tValues = tryItABunch(algo, startN=10, endN=end,stepSize=10)

plt.plot(nValues, tValues, color="blue", label="HashMap Algorithm")
plt.axis([0, 1000, 10, 20])
plt.xlabel("Number of IATA Codes; n=1000")
plt.ylabel("Time(ms)")
plt.legend()
plt.title("Hashmap Algorithm: Number of IATA Codes vs. Time")
plt.show()
